...
class OnboardUserTeamInput(BaseModel):
    ...
    game_key: str = Field(description="Yahoo Fantasy game key from get_user_leagues (e.g., 'nhl', 'nfl')")
    league_id: int = Field(description="Yahoo Fantasy league ID from get_user_leagues (just the numeric ID, e.g., '26455')")
    team_name: str = Field(description="Yahoo Fantasy team name from get_user_leagues")
    team_id: int = Field(description="Yahoo Fantasy team ID from get_user_leagues (just the numeric team ID)")

@tool(args_schema=OnboardUserTeamInput)
def onboard_user_team(user_email: str, game_key: str, league_id: int, team_name: str, team_id: int) -> str:
    """
    ...
    try:
        ...
        league_name = str(league_settings.name) if hasattr(league_settings, 'name') else f'League {league_id}'
        league_type = str(league_settings.game_code) if hasattr(league_settings, 'game_code') else 'unknown'

        # Save league to database
        # Convert Settings object to dict for JSON serialization
        settings_dict = league_settings.to_json() if hasattr(league_settings, 'to_json') else str(league_settings)
        add_yahoo_league(
            league_id=league_id,
            game_key=game_key,
            league_name=league_name,
            league_type=league_type,
            league_settings=settings_dict if isinstance(settings_dict, str) else json.dumps(settings_dict)
        )
...
"""
        ...
        return f"Error saving team: {str(e)}"
